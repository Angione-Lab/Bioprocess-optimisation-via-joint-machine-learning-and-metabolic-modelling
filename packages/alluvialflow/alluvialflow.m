...
%     plot(zeros(2, numel(left_category_points)), [left_category_points; (left_category_points + left_category_sizes)], 'k', 'LineWidth',2);
...
%     plot(ones(2, numel(right_category_points)), [right_category_points; (right_category_points + right_category_sizes)], 'k', 'LineWidth',2);
...
%                       0.5  0  1 ];
    ...
    for k_left = 1:size(data, 1) % for each row
        color = patch_colors(find(data(k_left, :)), :);
%         color = patch_colors(mod(k_left,num_colors)+1, :);
    ...
    text(zeros(1, size(data, 1)) - 0.01, ...
         ...
         left_labels, 'FontSize', 6, 'HorizontalAlignment', 'right', 'VerticalAlignment', 'middle', 'Interpreter', 'none');
%          left_labels, 'FontSize', 12, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'Rotation', 90);
    ...
    text(ones(1, size(data, 2)) + 0.01, ...
         ...
         right_labels, 'FontSize', 6, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'middle', 'Interpreter', 'none');
%          right_labels, 'FontSize', 12, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'Rotation', 90);
    
    title(chart_title, 'FontSize', 8);
    set(gca, 'LineWidth', 0.7, 'FontName', 'Arial', 'XColor', 'k', 'YColor', 'k')
end % alluvialflow
