...
# Prepare data
fluxes <- read.xlsx("../data/results/end_pFBA_fluxes_exploration.xlsx")
DoE <- read.xlsx("../data/raw/DoE data exploration.xlsx")
quant.fluxes <- as.matrix(fluxes)
class(quant.fluxes) <- "numeric"
num_samples <- dim(quant.fluxes)[1]
num_fluxes <- dim(quant.fluxes)[2]
quant.DoE <- as.matrix(DoE)
class(quant.DoE) <- "numeric"
num_DoEs <- dim(quant.DoE)[2]



## PLSCA on permuted data
num_permutations <- 999
lambdas <- matrix(NA, num_permutations, 7)
for (i in c(1:num_permutations)) {
  ...
  center.normed.DoE <- expo.scale(quant.DoE)
  escofier.transformed.DoE <- cbind((1-center.normed.DoE)/2, (1+center.normed.DoE)/2)
  res <- tepPLSCA(escofier.transformed.fluxes, escofier.transformed.DoE, graphs = FALSE)
...
PLSCA.res <- tepPLSCA(escofier.transformed.fluxes, escofier.transformed.DoE, graphs = FALSE)
...
for (i in c(1:num_DoEs)) {
  if (sum(quant.DoE[, i] != getmode(quant.DoE[, i])) == 1) {
    idx[i] <- FALSE
  }
}
quant.DoE.1 <- quant.DoE[, idx]

## Jackknife sampling
bootstrap_flux_scores <- array(0, c(2*dim(quant.fluxes.1)[2], 2, num_samples))
bootstrap_DoE_scores <- array(0, c(2*dim(quant.DoE.1)[2], 2, num_samples))
for (i in c(1:num_samples)) {
  ...
  bootstrap_DoE <- quant.DoE.1[bootstrap_idx, ]
  # Rescale fluxes and DoE
  center.normed.fluxes <- expo.scale(bootstrap_fluxes)
  escofier.transformed.fluxes <- cbind((1-center.normed.fluxes)/2, (1+center.normed.fluxes)/2)
  center.normed.DoE <- expo.scale(bootstrap_DoE)
  escofier.transformed.DoE <- cbind((1-center.normed.DoE)/2, (1+center.normed.DoE)/2)
  res <- tepPLSCA(escofier.transformed.fluxes, escofier.transformed.DoE, graphs = FALSE)
  bootstrap_flux_scores[, , i] <- res$TExPosition.Data$fi[, c(1,2)]
  bootstrap_DoE_scores[, , i] <- res$TExPosition.Data$fj[, c(1,2)]
}
...
## Save all
save.image("../data/results/PLSCA_results_exploration_quant.RData")

## Export data frames for MATLAB
writeMat('../data/results/PLSCA_results_exploration.mat', fluxes_factor_scores=PLSCA.res$TExPosition.Data$fi, 
         ...
         DoE_variables=colnames(quant.DoE),
         bootstrap_rxns=colnames(quant.fluxes.1),
         bootstrap_DoE_variables=colnames(quant.DoE.1))
