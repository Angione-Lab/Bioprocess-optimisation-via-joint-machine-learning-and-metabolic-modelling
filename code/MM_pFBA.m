...
% homogenise rxn names of common reactions
...
iEC1356_Bl21DE3 = changeRxnBounds(iEC1356_Bl21DE3, excRxns, 10, 'u'); % unlimited export of all metabolites
iEC1372_W3110 = changeRxnBounds(iEC1372_W3110, nutrient_rxns, -10, 'l');
excRxns = iEC1372_W3110.rxns(findExcRxns(iEC1372_W3110));
iEC1372_W3110 = changeRxnBounds(iEC1372_W3110, excRxns(not(ismember(excRxns, nutrient_rxns))), 0, 'l'); % block import of metabolites not in medium
iEC1372_W3110 = changeRxnBounds(iEC1372_W3110, excRxns, 10, 'u'); % unlimited export of all metabolites

% get irreversible models
[iEC1356_Bl21DE3_irrev, ~, ~, ~,] = convertToIrreversible(iEC1356_Bl21DE3);
[iEC1372_W3110_irrev, ~, ~, ~,] = convertToIrreversible(iEC1372_W3110);
[rxns, idxs, ~] = unique([iEC1356_Bl21DE3_irrev.rxns; iEC1372_W3110_irrev.rxns]);
subSystems = [iEC1356_Bl21DE3_irrev.subSystems; iEC1372_W3110_irrev.subSystems];
...
iEC1372_W3110_irrev = changeObjective(iEC1372_W3110_irrev, 'BIOMASS_Ec_iJO1366_WT_53p95M');
FBAsolution = optimizeCbModel(iEC1372_W3110_irrev);
...
predicted_atp_pre = NaN(num_conditions, 1);
predicted_production_pre = NaN(num_conditions, 1);
...
for i = 1:num_conditions
    ...
    if DoE_and_offline_data.Host(i) == 1
        ...
        bioreactor_model = iEC1372_W3110_irrev;
        bioreactor_model = change_enzyme_activity(bioreactor_model, critical_temperatures, 37, ...
    ...
    if constraint_data.preinduction_glycerol_ub(i) < 0
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_b', -constraint_data.preinduction_glycerol_ub(i), 'l');
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_f', 0, 'b');
    else
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_f', constraint_data.preinduction_glycerol_ub(i), 'u');
    end
    if constraint_data.preinduction_glycerol_lb(i) < 0
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_b', -constraint_data.preinduction_glycerol_lb(i), 'u');
    else
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_f', constraint_data.preinduction_glycerol_lb(i), 'l');
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_b', 0, 'b');
    end
    
    % other metabolites' exchange constraints (values +- std)
    positive_idxs = constraint_data.preinduction_exchange_rates(i, :) + constraint_data.preinduction_exchange_rates_errors(i, :) > 0;
    bioreactor_model = changeRxnBounds(bioreactor_model, strcat(constraint_data.rxns(positive_idxs), repmat({'_f'}, [1, sum(positive_idxs)])), ...
        constraint_data.preinduction_exchange_rates(i, positive_idxs) + constraint_data.preinduction_exchange_rates_errors(i, positive_idxs), 'u');
    bioreactor_model = changeRxnBounds(bioreactor_model, strcat(constraint_data.rxns(~positive_idxs), repmat({'_b'}, [1, sum(~positive_idxs)])), ...
        -constraint_data.preinduction_exchange_rates(i, ~positive_idxs) - constraint_data.preinduction_exchange_rates_errors(i, ~positive_idxs), 'l');
    %
    positive_idxs = constraint_data.preinduction_exchange_rates(i, :) - constraint_data.preinduction_exchange_rates_errors(i, :) > 0;
    bioreactor_model = changeRxnBounds(bioreactor_model, strcat(constraint_data.rxns(positive_idxs), repmat({'_f'}, [1, sum(positive_idxs)])), ...
        constraint_data.preinduction_exchange_rates(i, positive_idxs) - constraint_data.preinduction_exchange_rates_errors(i, positive_idxs), 'l');
    ...
    predicted_atp_pre(i) = FBAsolution.v(strcmp(bioreactor_model.rxns, 'ATPM'));
    predicted_production_pre(i) = FBAsolution.v(strcmp(bioreactor_model.rxns, 'PRODUCTtranslation2'));
end

% Save results:
if save_results == true
    save('..\data\results\preinduction_pFBA_fluxes.mat', 'fluxes', 'rxns', 'subSystems')
    save('..\data\results\preinduction_pFBA_data.mat')
end
...
for i = 1:num_conditions
    ...
    % glycerol exchange constraints
    if constraint_data.postinduction_glycerol_ub(i) < 0
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_b', -constraint_data.postinduction_glycerol_ub(i), 'l');
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_f', 0, 'b');
    else
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_f', constraint_data.postinduction_glycerol_ub(i), 'u');
    end
    if constraint_data.postinduction_glycerol_lb(i) < 0
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_b', -constraint_data.postinduction_glycerol_lb(i), 'u');
    else
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_f', constraint_data.postinduction_glycerol_lb(i), 'l');
        bioreactor_model = changeRxnBounds(bioreactor_model, 'EX_glyc_e_b', 0, 'b');
    end
    
    % other metabolites' exchange constraints (values +- std)
    positive_idxs = constraint_data.postinduction_exchange_rates(i, :) + constraint_data.postinduction_exchange_rates_errors(i, :) > 0;
    bioreactor_model = changeRxnBounds(bioreactor_model, strcat(constraint_data.rxns(positive_idxs), repmat({'_f'}, [1, sum(positive_idxs)])), ...
        constraint_data.postinduction_exchange_rates(i, positive_idxs) + constraint_data.postinduction_exchange_rates_errors(i, positive_idxs), 'u');
    bioreactor_model = changeRxnBounds(bioreactor_model, strcat(constraint_data.rxns(~positive_idxs), repmat({'_b'}, [1, sum(~positive_idxs)])), ...
        -constraint_data.postinduction_exchange_rates(i, ~positive_idxs) - constraint_data.postinduction_exchange_rates_errors(i, ~positive_idxs), 'l');
    %
    positive_idxs = constraint_data.postinduction_exchange_rates(i, :) - constraint_data.postinduction_exchange_rates_errors(i, :) > 0;
    bioreactor_model = changeRxnBounds(bioreactor_model, strcat(constraint_data.rxns(positive_idxs), repmat({'_f'}, [1, sum(positive_idxs)])), ...
        constraint_data.postinduction_exchange_rates(i, positive_idxs) - constraint_data.postinduction_exchange_rates_errors(i, positive_idxs), 'l');
...
%%
fluxes = zeros(num_conditions, length(rxns));
predicted_atp_end = NaN(num_conditions, 1);
predicted_production_end = NaN(num_conditions, 1);
...
for i = 1:num_conditions
    ...
    [a, b] = ismember(rxns, bioreactor_model.rxns);
    fluxes(i, a) = FBAsolution.v(b(a));
    predicted_atp_end(i) = FBAsolution.v(strcmp(bioreactor_model.rxns, 'ATPM'));
    predicted_production_end(i) = FBAsolution.v(strcmp(bioreactor_model.rxns, 'PRODUCTtranslation2'));
end
