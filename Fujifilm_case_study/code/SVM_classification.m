...
feature_cases = repmat({'DoE', 'concentrations', 'pFBA', 'MEP'}, 1, 1); % 'DoE', 'concentrations', 'pFBA', 'MEP' 
phase_cases = repmat({'pre', 'post', 'end', 'post'}, 1, 1); % 'pre', 'post', 'end'
num_samples = 22;
save_results = true;



for f_case = 4:length(feature_cases)
    ...
    num_cv_repetitions = 10; % for model evaluation robustness
    ...
    %%
    if save_results
        save(['..\data\results\SVM_results_' features '_' phase], 'SVM_results');
    end
end
